...
if __name__ == "__main__":
    ...
    for i in range(0, 5):
        if i == 0:
            pl = PowerLogger(interval=0.5, nodes=list(filter(lambda n: n[0].startswith('module/'), getNodes())))
            pl.start()
    ...
    output_dict['total_energy_consumption'] = avg_energy_consumption/5
    output_dict['avg_idle_power'] = avg_idle_power/5
    output_dict['avg_fl_power'] = avg_fl_power/5
    output_dict['epoch_times'] = [time/5.0 for time in avg_epoch_times]
